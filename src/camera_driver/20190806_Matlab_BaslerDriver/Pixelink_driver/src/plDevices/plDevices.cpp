		...
		case 1:		if (!strcmp(task, "print"))
					{
						...
						{
							mexPrintf("Device nr: %d --> %d, %d: GrabColorConversion = %d, GrabOutputType = %d\n", i, deviceArray[i].serialNumber, deviceArray[i].deviceID, deviceArray[i].grabColorConversion, deviceArray[i].grabOutputType);
						}
					}
					else
					{
		...
		case 3:		if (!strcmp(task, "add"))
					{
						...
						deviceArray[deviceCount].grabColorConversion = BAYER_3BY3_COLOR;
					...
					{	/* allocate memory for parName string and get the 3rd argument */
						...
						if (!strcmp(parName, "GrabColorConversion"))
						{
							...
							for (i = 0; i < deviceCount;i++)
							{
								if (deviceArray[i].serialNumber == serialnumber)
								{
									deviceArray[i].grabColorConversion = (U32)mxGetScalar(prhs[3]);
									t = 0;
									break; /* found a matching serialnumber, end search */
								}
							}
