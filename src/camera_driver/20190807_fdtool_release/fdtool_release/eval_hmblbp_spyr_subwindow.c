/*
  ...
  Evaluate model on image I of fast Histogram of MBLBP features through Histogram Integral and trained by Linear SVM  

  Usage
  ------

  [fx , y , H , IIR , R]               = eval_hmblbp_spyr_subwindow(I , [model]);
  ...
  model                                 Trained model structure
			 ...
			 addbias                    Use bias or not for prediction (1/0)
			 lambda                     Assign label with the formula y_est = sign(W'x - lambda) (default lambda = 0)
  ...
  H                                     Histogram of MBLBP computed for image I through fast Histogram Integral ((1+improvedLBP)*Nbins*nH*nscale) x 1)
  IIR                                   Integral Images for each bin and scale (Ny x Nx x(1+improvedLBP)*Nbins*nscale) in UINT32 format
  R                                     MBLBP maps per bin value (Ny x Nx x(1+improvedLBP)*Nbins*nscale) in UINT8 format

  To compile
  ----------

  mex  -g eval_hmblbp_spyr_subwindow.c

  mex  eval_hmblbp_spyr_subwindow.c

  mex  -f mexopts_intel10.bat eval_hmblbp_spyr_subwindow.c

  If OMP directive is added, OpenMP support for multicore computation

  mex -v -DOMP -f mexopts_intel10.bat eval_hmblbp_spyr_subwindow.c "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_core.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_c.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_thread.lib" "C:\Program Files\Intel\Compiler\C++\10.1.013\IA32\lib\libiomp5md.lib"

  or with the matfx option

  mex  -Dmatfx -f mexopts_intel10.bat eval_hmblbp_spyr_subwindow.c

  If OMP directive is added, OpenMP support for multicore computation

  mex -v -DOMP -Dmatfx -f mexopts_intel10.bat eval_hmblbp_spyr_subwindow.c "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_core.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_c.lib" "C:\Program Files\Intel\Compiler\11.1\065\mkl\ia32\lib\mkl_intel_thread.lib" "C:\Program Files\Intel\Compiler\C++\10.1.013\IA32\lib\libiomp5md.lib"


  mex -v -DOMP  eval_hmblbp_spyr_subwindow.c "C:\Program Files (x86)\Intel\Composer XE 2013\mkl\lib\intel64\mkl_core.lib"  "C:\Program Files (x86)\Intel\Composer XE 2013\mkl\lib\intel64\mkl_intel_thread.lib"
...
%  model.spyr               = [1 , 1 , 1 , 1 , 1 ; 1/2 , 1/2 , 1/4 , 1/4 , 1/4];
 ...
 elseif(model.maptable == 3)
  ...
  [fx , yfx , H , IIR , R]  = eval_hmblbp_spyr_subwindow(I , model);
  LBP                       = sum(double(R).*vect(ones(size(I,1),1),ones(1,size(I,2)),:) , 3);
  H1                        = mlhmslbp_spyr(I , model );

  H2                        = zeros(model.Nbins*(1+model.improvedLBP)*model.nH*length(model.scale) , 1);
...
%  model.spyr               = [1 , 1 , 1 , 1 ; 1/2 , 1/2 , 1/2 , 1/2 ];
  ...
  [fx, yfx , H ]           = eval_hmblbp_spyr_subwindow(Icrop , model);
  ...
  [fx1 , yfx1 , H1]  = eval_hmblbp_spyr_subwindow(Icrop_interp , model);
  ...
  for i = vect
	 ...
	 yfx1                   = sign(fx1(co));
     [fx2(co) , yfx2 , H2]  = eval_hmblbp_spyr_subwindow(Itemp , model);
     co                     = co+1;
  end
...
int eval_hmblbp_spyr_subwindow(unsigned int * , double * , int , int , int , int , double  , struct model   , double *);
int eval_hmblbp_spyr_subwindow_hom(unsigned int * , double * , int , int , int , int , double  , struct model   , double *);
void homkertable(struct model  , double * );
void eval_hmblbp_spyr(unsigned char * , int , int , struct model , double * , double *, double *, unsigned int * , unsigned char *);
/*-------------------------------------------------------------------------------------------------------------- */
void mexFunction( int nlhs, mxArray *plhs[] , int nrhs, const mxArray *prhs[] )
{
	...
	if ((nrhs > 1) && !mxIsEmpty(prhs[1]) )   
	{
		...
		{
			detector.w                    = (double *)mxMalloc(detector.Nbins*detector.nH*detector.nscale*sizeof(double));
		}
	...
	eval_hmblbp_spyr(I , Ny , Nx  , detector , y , fx , H , IIR , R);
...
/*----------------------------------------------------------------------------------------------------------------------------------------- */
void eval_hmblbp_spyr(unsigned char *I , int Ny , int Nx  , struct model detector , double *yfx , double *fx , double *H , unsigned int *IIR , unsigned char *R)
{
	unsigned int *II , *Itemp;
	int nscale = detector.nscale  , nH = detector.nH;
	int Nbins = detector.Nbins , cs_opt = detector.cs_opt;
	int maptable = detector.maptable , improvedLBP = detector.improvedLBP , n = detector.n;
...
#pragma omp for	nowait	
	...
	if(n > 0)
	{
		yest      = eval_hmblbp_spyr_subwindow_hom(IIR , H  , Ny , Nx  , Nbins , NbinsnscalenH  , maxfactor , detector , fx);
	}
	else
	{
		yest      = eval_hmblbp_spyr_subwindow(IIR , H  , Ny , Nx  , Nbins , NbinsnscalenH  , maxfactor , detector , fx);
	}
...
/*----------------------------------------------------------------------------------------------------------------------------------------- */
int eval_hmblbp_spyr_subwindow(unsigned int *IIR , double *H  , int Ny , int Nx , int Nbins , int  NbinsnscalenH  , double maxfactor , struct model detector  , double *fx)
{
	double *w = detector.w , *spyr = detector.spyr;
	double clamp = detector.clamp , lambda = detector.lambda;
	...
	fx[0] = sum;
	return (sign(sum - lambda));
}

/*----------------------------------------------------------------------------------------------------------------------------------------- */
int eval_hmblbp_spyr_subwindow_hom(unsigned int *IIR , double *H  , int Ny , int Nx , int Nbins , int  NbinsnscalenH  , double maxfactor , struct model detector  , double *fx)
{
