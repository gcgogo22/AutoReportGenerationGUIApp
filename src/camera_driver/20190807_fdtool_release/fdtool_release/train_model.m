...
%                   negresize        Resize negative images to dimsItraining size
...
%                   keepnegsize      Keep negatives image size, i.e take all the negative image and not a patch of it (default keepnegsize = 0)
%                                             
%                                                Features
%
...
%                   useweight        Use weighting in the SVM training (default useweight = 0)
%
...
% options.positives_path     = fullfile(pwd , 'images' , 'faces' , 'test' , 'positives');
% options.negatives_path     = fullfile(pwd , 'images' , 'faces' , 'train' , 'negatives');
% %options.posext             = {'png'};
% options.posext             = {'pgm'};
% options.negext             = {'jpg'};
...
if(~any(strcmp(fieldnames(options) , 'negresize')))
    options.negresize           = 0;
end
...
if(~any(strcmp(fieldnames(options) , 'keepnegsize')))
    options.keepnegsize         = 0;
end
...
if(~any(strcmp(fieldnames(options) , 'useweight')))
    options.useweight           = 0;
end
...
elseif(options.typeclassifier == 2)
    if(options.useweight)
        npos                                                          = sum(options.ytrain==1);
        nneg                                                          = length(options.ytrain) - npos;
        options.wpos                                                  = nneg/npos;
        paramclassifstr                                               = sprintf('-q -s %d -B %f -c %f -w1 %f' , options.s , options.B , options.c , options.wpos);
    else
        paramclassifstr                                               = sprintf('-q -s %d -B %f -c %f' , options.s , options.B , options.c);
    end
    model                                                             = train_dense(options.ytrain' , options.Xtrain , paramclassifstr , 'col');
    options.w                                                         = model.w;
    options.fxtrain                                                   = options.w(1:end-1)*options.Xtrain;
    if(options.addbias)
...
%        model                                                         = train_dense(options.ytrain' , options.Xtrain , sprintf('-s %d -B %d -c %f' , options.s , options.B , options.c) , 'col');
        options.w                                                     = model.w;
        options.fxtrain                                               = options.w(1:end-1)*options.Xtrain;
        if(options.addbias)
            options.fxtrain                                           = options.fxtrain + options.w(end);
