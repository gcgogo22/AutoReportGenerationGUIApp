function plot_current_eff(obj)
    ...
    for i = 1:cond_num
        for j = 1:dev_num
            ...
            eff_data = tb_data{4+j, obj.ind(end):1:obj.ind(end)+ length(obj.current)-1};
            plot(obj.current, eff_data*100, 'DisplayName', leg, 'Color', colors(i, :), 'Marker', marker, 'MarkerSize', 10, ...
                'LineWidth', 2);            
        end 
    end
    hold off; 
