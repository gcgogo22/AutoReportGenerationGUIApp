function plot_current_thermres(obj)
    % This function plot the thermal resistance vs. current
    ...
    fig_hd = figure('Name', 'Power vs. Thermal Res', 'Position', [1987 45 1031 773]); %#ok<NASGU>
    hold on;
    for i = 1:cond_num
        for j = 1:dev_num
            ...
            therm_res_data = tb_data{4+j, obj.ind(7):1:obj.ind(7)+ length(obj.current)-1};
            plot(obj.current, therm_res_data, 'DisplayName', leg, 'Color', colors(i, :), 'Marker', marker, 'MarkerSize', 10, ...
                'LineWidth', 2);            
        end 
    end
    hold off; 
