function plot_current_vol(obj)
    % This function plot the voltage vs. current
    ...
    fig_hd = figure('Name', 'Voltage vs. Current', 'Position', [1987 45 1031 773]); %#ok<NASGU>
    hold on;
    for i = 1:cond_num
        for j = 1:dev_num
            ...
            vol_data = tb_data{4+j, obj.ind(2):1:obj.ind(2)+ length(obj.current)-1};
            plot(obj.current, vol_data, 'DisplayName', leg, 'Color', colors(i, :), 'Marker', marker, 'MarkerSize', 10, ...
                'LineWidth', 2);            
        end 
    end
    hold off; 
