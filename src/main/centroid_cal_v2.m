...
fld = 'C:\Trumpf_photonics\DM17_proj\DM17_data\20190129_DM17moduleBld2\Point_Shift_atDiffCurrRange\02152019_tabBaseline\30064071';
cd(fld);
curr_fld = pwd;
newfl = 'Processed';
mkdir(newfl);

files = ls('*raw.png'); % the file order is not correct;
...
%This function reorder the file from small current to large current
    len = size(files,1);
    for i = 1:len
        ind = strfind(files(i, :), 'r');
        temp = files(i, :);
        current(i) = str2double(temp(1:(ind-1)));
    end
